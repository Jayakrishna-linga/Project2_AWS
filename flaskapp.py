...
def registration():
    ...
    if request.method == 'POST' and str(request.form['usn']) !="" and str(request.form['pwd']) !="" and str(request.form['fn']) !="" and str(request.form['ln']) !="">
        ...
        if res:
            ...
            sq1 = execute_query("""INSERT INTO users (username, password, firstname, lastname, email) values (?, ?, ?, ?, ? )""", (username, password, firstname, las>
            commit()
            sq2 = execute_query("""SELECT firstname,lastname,email  FROM users WHERE Username  = (?) AND Password = (?)""", (username, password ))
            if sq2:
                for row in sq2:
                    return responsePage(row[0], row[1], row[2])
...
def responsePage(firstname, lastname, email):
